# ≡ƒîÇ VortexRAG ΓÇö Enterprise Codebase Intelligence & GraphRAG Swarm
...
## ≡ƒÆí 2. Problem Statement & Motivation
### The Problem
...
## Γ£¿ 3. Key Features
* **≡ƒî▓ AST Semantic Chunking:** Uses `tree-sitter` grammars to parse Python, TypeScript, Go, Java, and JavaScript into clean, standalone AST class and function nodes.
* **≡ƒò╕∩╕Å Neo4j GraphRAG Call-Graphs:** Stores code containment relationships and call-graph dependencies (`CONTAINS`, `CALLS`). Retrieves structural siblings dynamically to prevent hallucination.
* **≡ƒº¼ BM25 + Dense Hybrid Search (RRF):** Fuses dense vector embeddings (Voyage AI) with sparse keyword queries using Reciprocal Rank Fusion (RRF).
* **≡ƒñû Stateful LangGraph Parallel Swarm:** Routes retrieved chunks to parallel **Security** and **Performance** agents before joining at a lead **Compliance Architect** node.
* **ΓÜí Redis Semantic Cache:** Compares incoming queries mathematically against a vector cache using cosine similarity, serving cache hits in under 15ms.
* **≡ƒöä Smart Delta Ingestion:** Uses Git diffs to delete, chunk, and upsert *only* modified files, completing ingestion in seconds.
* **≡ƒöÉ Go OAuth2 Microservice:** A high-speed, secure, SQLite-backed auth microservice implemented in Go that generates and verifies stateless JWT access tokens.
* **≡ƒîè Server-Sent Events (SSE):** Real-time, word-by-word streaming of agent progress states and token responses using Next.js 14 and FastAPI.

## ≡ƒöù Live Demo Link
≡ƒôì **[Launch VortexRAG Enterprise Dashboard](https://vortex-codebase-intelligence.vercel.app/)**

---

## ≡ƒôè 4. Demo & Architecture Diagram

### Swarm Execution Demo
```text
≡ƒöì Searching Vector Database... -> [CACHE MISS]
≡ƒ¢í∩╕Å Grading retrieved code relevancy... -> [3 Chunks Kept]
≡ƒ¢í∩╕Å [Swarm] Security Agent auditing vulnerabilities... (Parallel)
ΓÜí [Swarm] Performance Agent profiling bottlenecks... (Parallel)
≡ƒîÇ [Swarm] Lead Architect synthesizing unified report... (Join)
≡ƒîè Stream: # VortexRAG Architect Review...
```
...
## ≡ƒÅ¢∩╕Å 5. High-Level System Architecture
...
## ≡ƒÅâΓÇìΓÖé∩╕Å 6. End-to-End Workflow
...
## ≡ƒº¼ 7. Retrieval-Augmented Generation (RAG) Pipeline

VortexRAG utilizes a stateful self-correction LangGraph cycle designed to prevent hallucinations:

```text
[Retriever Node] Γ₧ö [Grader Node] Γ₧ö Relevant? 
                                    Γö£ΓöÇΓöÇ Yes: [Swarm Nodes (Parallel)] Γ₧ö [Synthesize] Γ₧ö END
                                    ΓööΓöÇΓöÇ No:  [Query Rewriter] Γ₧ö [Retriever Node] (Loop)
```
...
## ≡ƒî▓ 8. Data Ingestion & Chunking Strategy
...
## ≡ƒº¼ 9. Vector DB & Hybrid Search Design (RRF)
...
## ≡ƒò╕∩╕Å 10. Neo4j GraphRAG & Call-Graph Context Injection
...
## ≡ƒñû 11. Parallel Multi-Agent Swarm (LangGraph)
VortexRAG deploys a highly advanced **LangGraph parallel-fan-out and merge** multi-agent network:

```text
               ΓöîΓöÇΓöÇ ≡ƒ¢í∩╕Å Security Auditor Agent ΓöÇΓöÇΓöÉ
[Retrieve/Grade]                              Γö£ΓöÇΓöÇΓ₧ö [Compliance Architect] Γ₧ö END
               ΓööΓöÇΓöÇ ΓÜí Performance Profiler ΓöÇΓöÇΓöÇΓöÇΓöÿ
```

### The Agents
* **≡ƒ¢í∩╕Å Security Auditor Agent (Parallel Node):** A specialized CISSP-certified agent analyzing codebase chunks for secrets leakage, SQL injections, insecure packages, and JWT logic bypasses.
* **ΓÜí Performance Profiler Agent (Parallel Node):** A specialized Principal Architect analyzing big-O algorithm complexity, CPU/memory hotspots, redundant DB queries, and open connection leaks.
* **≡ƒîÇ Compliance Architect Agent (Join Synthesizer):** Collects the core code context, merges the parallel Security Audit and Performance Profile, and compiles the final unified markdown report.

---

## ΓÜí 12. Redis Semantic Caching Strategy
...
## ≡ƒöÉ 13. Go Authentication Microservice
...
Γöé   Γöé   Γöé   ΓööΓöÇΓöÇ v1/
Γöé   Γöé   Γöé       Γö£ΓöÇΓöÇ endpoints/
Γöé   Γöé   Γöé       Γöé   Γö£ΓöÇΓöÇ auth.py          # Go-Auth integration bindings
Γöé   Γöé   Γöé       Γöé   Γö£ΓöÇΓöÇ ingest.py        # Trigger full & smart incremental delta syncs
Γöé   Γöé   Γöé       Γöé   ΓööΓöÇΓöÇ query.py         # Real-time SSE streaming agent query gateway
Γöé   Γöé   Γöé       ΓööΓöÇΓöÇ api.py
Γöé   Γöé   Γö£ΓöÇΓöÇ core/
Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ config.py            # Global env parser
Γöé   Γöé   Γöé   ΓööΓöÇΓöÇ security.py          # FastAPI JWT token validators
Γöé   Γöé   Γö£ΓöÇΓöÇ db/
Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ neo4j.py             # Neo4j Driver pool
Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ qdrant.py            # Qdrant client pool
Γöé   Γöé   Γöé   ΓööΓöÇΓöÇ redis.py             # Redis Vector DB semantic cache pool
Γöé   Γöé   ΓööΓöÇΓöÇ services/
Γöé   Γöé       Γö£ΓöÇΓöÇ ast_chunker.py       # tree-sitter AST parser
Γöé   Γöé       Γö£ΓöÇΓöÇ embedder.py          # Voyage AI API wrapper
Γöé   Γöé       Γö£ΓöÇΓöÇ graph_builder.py     # Neo4j graph generator
Γöé   Γöé       Γö£ΓöÇΓöÇ hybrid_search.py     # dense Qdrant + local BM25 + mathematical RRF
Γöé   Γöé       Γö£ΓöÇΓöÇ ingestion.py         # Full & smart incremental delta pipelines
Γöé   Γöé       ΓööΓöÇΓöÇ rag_agent.py         # LangGraph Collaborative Multi-Agent Swarm
Γöé   ΓööΓöÇΓöÇ main.py
Γö£ΓöÇΓöÇ frontend/
Γöé   Γö£ΓöÇΓöÇ src/
Γöé   Γöé   Γö£ΓöÇΓöÇ app/
Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ dashboard/
Γöé   Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ ingest/page.tsx  # Ingest monitoring panel
Γöé   Γöé   Γöé   Γöé   Γö£ΓöÇΓöÇ pr-review/       # PR Agent trigger
Γöé   Γöé   Γöé   Γöé   ΓööΓöÇΓöÇ query/page.tsx   # Premium search & swarm monitor UI
Γöé   Γöé   Γöé   ΓööΓöÇΓöÇ page.tsx
Γöé   ΓööΓöÇΓöÇ package.json
ΓööΓöÇΓöÇ go-auth/                         # Go Auth microservice
    Γö£ΓöÇΓöÇ db/
    Γö£ΓöÇΓöÇ handlers/
    Γö£ΓöÇΓöÇ models/
    Γö£ΓöÇΓöÇ main.go
...
## ΓÜÖ∩╕Å 16. Installation & Setup Guides
...
## ≡ƒº¬ 18. Testing & Performance Benchmarks
...
## ≡ƒÆí 19. Challenges Faced & Lessons Learned
...
## ≡ƒö« 20. Future Enhancements & Roadmap
